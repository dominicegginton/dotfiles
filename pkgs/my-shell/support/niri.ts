...
export class Niri extends GObject.Object {
  ...
  constructor() {
    ...
    // Start the event stream - it will send initial WorkspacesChanged and WindowsChanged events
    subprocess(
  ...
  onWorkspacesChanged(workspaces: Workspace[]) {
    ...
    // Update workspaces but preserve windows and the correct focus state
    ...
    this.workspaces = workspaces
      ...
      .map((ws) => {
        ...
        return {
          ...
          windows: existing?.windows || [], // Preserve a custom windows array
        };
      });
  ...
  onWindowsChanged(windows: Window[]) {
    ...
    // Update a windows array while preserving the correct focus state and normalising optional fields
  ...
  updateWorkspaceWindows() {
    ...
    this.workspaces = this.workspaces.map((workspace) => {
      ...
      // Apply the current focus state to all windows while preserving order
  ...
  onWindowOpenedOrChanged(window: Window) {
    ...
    // Update or add the window in our window array
    const existingIndex = this.windows.findIndex((w) => w.id === window.id);

    if (existingIndex >= 0) {
      // Update existing windows, preserving focus state from our active window tracking
      ...
      // Add a new window, setting focus state based on current active window
      const activeWindowId = this.activeWindowId.get();
      this.windows.push({
        ...normalizedWindow,
        is_focused: activeWindowId !== -1 && window.id === activeWindowId,
  ...
  onWindowClosed(windowId: number) {
    // Remove the window from our window array
    this.windows = this.windows.filter((w) => w.id !== windowId);

    // Clear the active window if it was the closed one
  ...
  onWindowUrgencyChanged(windowId: number, urgent: boolean) {
    // Update the urgency state for the window in both window array and workspace windows
    this.windows = this.windows.map((window) => ({
      ...window,
