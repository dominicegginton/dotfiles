...
if [[ -z "$TARGET_HOST" ]]; then
  ...
  ls -1 hosts/nixos/*/default.nix | cut -d'/' -f3 | grep -v iso
  exit 1
fi
...
if [[ -z "$TARGET_USER" ]]; then
  ...
  ls -1 hosts/nixos/modules/users/ | grep -v -E "nixos|root"
  exit 1
fi

if [ ! -e "hosts/nixos/$TARGET_HOST/disks.nix" ]; then
  echo "ERROR! $(basename "$0") could not find the required nixos/hosts/$TARGET_HOST/disks.nix"
  exit 1
fi

if grep -q "data.keyfile" "hosts/nixos/$TARGET_HOST/disks.nix"; then
